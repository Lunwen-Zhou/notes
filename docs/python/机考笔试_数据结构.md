...
张量长度组合 3 + 7，4 + 6，5 + 6，5 + 7，6 + 7，3 + 4 + 5，... 都大于等于10，满足运行的基本条件

对于 3 + 7 来说，3 的最小代价是1，7 的最小代价是5，3 + 7 的最小代价是6，经过其他组的比较，达成目标的最小代价是6，所以输出 6
...
- 选第 $i$ 个张量，大小是 `size`，代价是 `cost`
  ...
  新释放量变成 $j+size$，但超过目标 $m$ 后都统一压成 $m$：
  $$
  new_j = \min(m, j+size)
  $$
  故：
  ...
  >如果 $j+size \leq m$，这个状态转移方程很好理解，就是
  >$$
  >dp[i][j + size] = \min(dp[i][j + size],\; dp[i-1][j] + cost)
  >$$
  >就是从状态 $dp[i-1][j]$ 转移到 $dp[i][j+size]$

  因此需要先判断状态 $dp[i-1][j]$ 是否是可达的，**只有当状态是可达的，我们才允许它转移**
